...
impl ConnectionPool {
  ...
  pub async fn remove(&self, room: &str, sender: &mpsc::Sender<Message>) {
    if let Some(senders) = self.connections
      .write().await
      .get_mut(room) {
...
impl GameWebSocket
{
  ...
  )
  {
   let game_state = Game::instance();
    ...
    GameWebSocket::connection_driver(room.clone(), tx, game_state, ws_tx, ws_rx, rx).await;
          ...
          msg_result = ws_rx.next() => {
              match msg_result {
                  Some(Ok(msg)) => {
                      if let Ok(text) = msg.to_str() {
                              ...
                              serde_json::from_str::<ClientMessage>(text)
                          {
                              ...
                              if let Some(room_state) = game_state.get_room_state(&room) {
                  ...
                  None => {
                      ...
                      break; // Exit loop when connection is closed
                  }
              }
          },
